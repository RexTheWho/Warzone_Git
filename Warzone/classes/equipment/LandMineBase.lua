LandMineBase = LandMineBase or class(UnitBase)
LandMineBase._BULLET_STORM = {
	2.5,
	10
}
local dec_mul = 10000
local unit_name = "units/pd2_mod_warzone/equipment/gen_equipment_landmine/gen_equipment_landmine"

function LandMineBase.spawn(pos, rot, ammo_upgrade_lvl, peer_id, bullet_storm_level)
...
function LandMineBase:set_server_information(peer_id)
...
function LandMineBase:server_information()
	return self._server_information
end

function LandMineBase:init(unit)
...
function LandMineBase:get_name_id()
	return "ammo_bag"
end

function LandMineBase:_clbk_validate()
...
function LandMineBase:sync_setup(ammo_upgrade_lvl, peer_id, bullet_storm_level)
...
function LandMineBase:setup(ammo_upgrade_lvl, bullet_storm_level)
...
function LandMineBase:update(unit, t, dt)
	self:_check_body()
end

function LandMineBase:_check_body()
...
function LandMineBase:server_set_dynamic()
	self:_set_dynamic()
...
function LandMineBase:sync_net_event(event_id)
	self:_set_dynamic()
end

function LandMineBase:_set_dynamic()
	self._is_dynamic = true

	self._unit:body("dynamic"):set_enabled(true)
end

function LandMineBase:take_ammo(unit)
	if self._empty then
		return false, false
	end
...
function LandMineBase:_set_visual_stage()
...
function LandMineBase:sync_ammo_taken(amount)
...
function LandMineBase:_take_ammo(unit)
	local taken = 0
...
function LandMineBase:_set_empty()
	self._ammo_amount = 0
	self._empty = true
...
function LandMineBase:save(data)
	local state = {
		ammo_amount = self._ammo_amount,
		is_dynamic = self._is_dynamic,
		bullet_storm_level = self._bullet_storm_level
	}
	data.LandMineBase = state
end

function LandMineBase:load(data)
	local state = data.LandMineBase
...
function LandMineBase:round_value(val)
	return math.floor(val * dec_mul) / dec_mul
end

function LandMineBase:destroy()
	if self._validate_clbk_id then
		managers.enemy:remove_delayed_clbk(self._validate_clbk_id)
...
CustomLandMineBase = CustomLandMineBase or class(LandMineBase)

function CustomLandMineBase:init(unit)
	CustomLandMineBase.super.init(self, unit)
...
function CustomLandMineBase:_set_empty()
	self._empty = true
